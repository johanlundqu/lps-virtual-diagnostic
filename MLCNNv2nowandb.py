...
def ssim_loss(y_true, y_pred):
    ssiml=np.array([ssim(y_true[i], y_pred[i]) for i in tf.range(len(y_true))])
...
def buildModel(model,modelname,inputs,labels,trainb=True): #For 200x200 images, multiply stuff by 4
    ...
    if trainb:
        history=model.fit(inputs,labels,batch_size=250,epochs=750, validation_split=0.01, callbacks=[mcp_save], verbose=10)
